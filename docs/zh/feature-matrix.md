...
| OpenClaw agent runtime | Core | 运行 agents、sessions、channels、cron jobs 和频道级 prompt profiles | `docs/openclaw-setup.md`、`docs/openclaw-config-guide.md`、`examples/openclaw/` | 用 examples 配置 `~/.openclaw/openclaw.json` | `openclaw status`；在每个配置频道发测试消息；确认 channel bindings 路由到预期 agent workspace |
