...
| OpenClaw Brain OS install pack | Core | 提供可安装的 OpenClaw 分发包，包含 config patch 模板、workspace bootstrap、skill manifest 和 canonical vault-template 接入 | `packs/openclaw-brain-os/` | 从 `packs/openclaw-brain-os/README.md` 开始；PR2 支持 --check/--dry-run，以及保守的 safe apply/rollback | 运行 `bash packs/openclaw-brain-os/tests/smoke.sh`；验证 JSON 模板和无私有信息 |
