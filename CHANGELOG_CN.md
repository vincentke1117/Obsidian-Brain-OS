...
- **`examples/openclaw/openclaw.channel-prompts.example.json`** — 新增 OpenClaw 频道级 prompt profile 示例，展示如何通过频道到 agent 的 bindings 与独立 workspace 实现不同频道不同提示词。
- **`docs/agent-bootstrap-guide.md`** / **`docs/zh/agent-bootstrap-guide.md`** — 新增中英双语指南，说明如何编写短小的 `AGENTS.md`，以及如何组织按需读取的 `references/` 操作手册。

### 变更 / Changed
- **`docs/feature-matrix.md`** / **`docs/zh/feature-matrix.md`** — 将 Agent bootstrap + references 加入 Core 功能清单，并补充安装与验证方法；同时明确 OpenClaw 频道绑定可用于验证 route-to-workspace prompt profiles。
